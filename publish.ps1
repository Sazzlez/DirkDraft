<#
.SYNOPSIS
    Builds DraftPilot and puts a shortcut on the desktop.
...
$root = $PSScriptRoot
if (-not $OutputDirectory) { $OutputDirectory = Join-Path $root 'build\DraftPilot' }
...
$running = Get-Process DraftPilot -ErrorAction SilentlyContinue
if ($running) {
    Write-Host 'DraftPilot laeuft und wird beendet, damit die Dateien nicht gesperrt sind.'
...
$exe = Join-Path $OutputDirectory 'DraftPilot.exe'
...
$linkPath = Join-Path $desktop 'DraftPilot.lnk'
...
if (Test-Path $linkPath) { [System.IO.File]::Delete($linkPath) }
